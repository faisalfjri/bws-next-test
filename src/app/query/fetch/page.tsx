...
const API_BASE = "https://sisda.bwssumatera1.net/api/articles";
...
export default function FetchPage() {
  ...
  const load = async (url: string) => {
    ...
    try {
      ...
      setArticles((prev) => {
        ...
        return [...prev, ...json.data.filter((a) => !ids.has(a.id))];
      });
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      >
        <span className="inline-block mb-4 rounded-full bg-gray-100 px-4 py-1.5 text-xs font-medium text-gray-600">
          Native Fetch
        </span>
        ...
        <p className="mt-3 sm:mt-4 text-sm sm:text-base text-gray-500 px-2">
          Menggunakan native <code className="bg-gray-100 px-1.5 py-0.5 rounded text-sm">fetch()</code> API
        </p>
      </motion.section>
